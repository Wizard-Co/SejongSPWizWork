    ...
    public partial class frm_tprc_Work_U : Form
    {
                                   ...
                                   double LabelPaper_Count, double LabelPaper_OneMoreQty, double QtyperBox)
        {
            try
            {
                ...
                //'--------------------------------------------------------------------------------
                // 첫 공정이다.
                if (InstDetSeq == 1 && (LabelPrintYN == "Y") || LastArticle.Contains("조립")) //2022-11-11 마지막 품명에 조립이란 단어가 들어가면 라벨 계속 생성
                {
                    int mInstDetSeq = 0;
                    long nQtyPerBox = 0;
            ...
            try
            {
                ...
                for (int i = 0; i < list_TWkResult.Count; i++)
                {
                    ...
                    if (list_TWkResult[0].JobGbn == "1")
                    {
                        ...
                        #region 주석 금형 등록을 노란 박스 만들면서 등록함 2022-01-25
                        // '************************************************************************************************
                        //'                              금형 등록 시   //xp_wkResult_iWkResultMold
                        //'*************************************************************************************************

                        //2022-01-25 Frm_tprc_Main.list_tMold.Count > 0 이면 금형등록이 되어 있음
                        if (Frm_tprc_Main.list_tMold.Count > 0)
                        {
                            ...
                            sqlParameter6.Add("MoldID", Frm_tprc_Main.list_tMold[i].sMoldID);
